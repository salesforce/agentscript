...
// ===========================================================================
// Custom (non-standard) connection types
//
// Connection blocks with names not in the standard list (messaging,
// service_email, slack, telephony, voice, customer_web_client) pass the block
// name through verbatim as the surface_type. There is no separate `name`
// field — `surface_type` IS the connection name (case-preserved).
// ===========================================================================

describe('custom connection types', () => {
  it('should compile custom connection with full features (inputs, response_formats, response_actions)', () => {
    const source = agentSource(`
connection penguin:
...
`);
    ...
    const surface = findSurface(result, 'penguin');
    expect(surface).toBeDefined();
    expect(surface?.surface_type).toBe('penguin');
    expect(surface?.input_parameters).toHaveLength(1);
    expect(surface?.response_formats).toHaveLength(1);
    expect(surface?.response_actions).toHaveLength(1);
  });
...
`);
    ...
    // Unknown connection names pass through verbatim (case-preserved).
    const surface = findSurface(result, 'MyAPIClient');
    expect(surface).toBeDefined();
    expect(surface?.input_parameters).toHaveLength(3);

    const inputsByName = new Map(
      surface?.input_parameters?.map(i => [i.developer_name, i])
    );
    ...
    expect(timeout?.data_type).toBe('double');
...
// ===========================================================================
// Response format compilation (response_formats / input_schema)
// ===========================================================================

describe('response_formats compilation', () => {
...
`);
    ...
    const surface = findSurface(result, 'messaging');
    expect(surface).toBeDefined();
    expect(surface?.response_formats).toBeDefined();
    expect(surface?.response_formats).toHaveLength(1);

    const format = surface?.response_formats?.[0];
    ...
    // No inputs → an empty object schema (input_schema is always emitted).
    expect(format?.input_schema).toBe('{"type":"object","properties":{}}');
  });
...
`);
  ...
  it('should canonicalize a prompt:// target to generatePromptResponse', () => {
    const source = agentSource(`
connection messaging:
    adaptive_response_allowed: True
    ...
    response_formats:
        prompt_format:
            description: "A prompt format"
            target: "prompt://MyPrompt"
`);
    const result = compileSource(source);

    const format = findSurface(result, 'messaging')?.response_formats?.[0];
    // Authored scheme `prompt` is emitted as `generatePromptResponse`.
    expect(format?.invocation_target_type).toBe('generatePromptResponse');
    expect(format?.invocation_target_name).toBe('MyPrompt');
  });
...
`);
    ...
    // is_required defaults to True, so every field is required here.
    expect(schema.required).toEqual(['name', 'count', 'score', 'active']);
  });
...
`);
    ...
    // is_required defaults to True, so `tags` is required despite no flag.
    expect(schema.required).toEqual(['message', 'choices', 'tags']);
  });
...
`);
    ...
    expect(surface?.input_parameters).toHaveLength(1);
    expect(surface?.response_formats).toHaveLength(2);
    expect(surface?.response_actions).toHaveLength(1);
    expect(surface?.outbound_route_configs).toHaveLength(1);
  });
});
...
// ===========================================================================
// Response format / connection source URI scheme validation
// ===========================================================================

describe('response_format source URI validation', () => {
...
`);
    ...
    const errors = getErrors(result);
    // Two invalid sources → two errors.
    ...
    // The correct format does not generate an error.
    ...
    expect(hasDiagnosticMatching(errors, 'SurfaceAction__InvalidFormat')).toBe(
      true
    );
...
`);
    ...
    const surface = findSurface(result, 'messaging');
    expect(surface?.response_formats).toHaveLength(1);
    expect(surface?.response_formats?.[0].source).toBe(
...
// ===========================================================================
// Nested object inputs compilation (JSON Schema)
// ===========================================================================
...
`);
  ...
  it('should compile label property to title with non-property subfields', () => {
    const source = agentSource(`
connection penguin:
    label: "Penguin"
    response_formats:
        mixed_format:
            ...
            inputs:
                # label compiles to JSON Schema title
                displayName: string
                    label: "Display Name"
...
`);
    ...
    // Case 1: label as PROPERTY → title
    ...
    // Case 2: nested fields compile into the array items schema
