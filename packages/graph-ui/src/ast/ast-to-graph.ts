...
import { findTopicBlock, type AgentScriptAST } from './ast-utils';
...
function extractTransitions(
  ...
  for (const stmt of statements) {
    if (stmt.__kind === 'TransitionStatement') {
      const transition = stmt as unknown as { clauses: Statement[] };
      for (const clause of transition.clauses) {
        if (clause.__kind === 'ToClause') {
          const toClause = clause as unknown as { target: unknown };
...
function extractTransitionsFromReasoningAction(
  ...
  for (const stmt of statements) {
    if (stmt.__kind === 'ToClause') {
      const toClause = stmt as unknown as { target: unknown };
      const target = resolveTransitionTarget(toClause.target);
      if (target) {
...
function buildDetailNodes(
  ...
  for (const stmt of statements) {
    if (stmt.__kind === 'TransitionStatement') {
      ...
      const runStmt = stmt as unknown as { target: unknown; body: Statement[] };
      ...
      const setStmt = stmt as unknown as {
        target: unknown;
