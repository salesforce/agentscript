...
export function SetNode({ data, selected }: NodeProps<GraphNode>) {
  const borderClass = getNodeBorderClass(selected, data.diagnostics);

  return (
    <div
      className={`min-w-40 max-w-70 rounded-xl border border-gray-300 bg-white shadow-md transition-colors duration-150 hover:border-gray-400 dark:border-zinc-700 dark:bg-zinc-800 dark:hover:border-zinc-600 ${borderClass}`}
    >
      <NodeHandles
        sides={DETAIL_SIDES}
