...
export function CompoundTopicNode({ data, selected }: NodeProps<GraphNode>) {
  const sections = ALL_SECTIONS;
  const borderClass = getNodeBorderClass(selected, data.diagnostics);

  return (
    <div
      className={`relative min-w-[200px] rounded-xl border border-gray-300 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-800 ${borderClass}`}
    >
      <NodeHandles
        sides={{
          top: { type: 'target' },
      ...
      {/* Header */}
      <div className="flex items-center gap-2.5 border-b border-gray-100 px-4 py-3 dark:border-zinc-700">
          ...
          <div
            ...
            className={`relative flex items-center justify-between px-4 py-2.5 ${
              idx < sections.length - 1
                ? 'border-b border-gray-50 dark:border-zinc-700'
                : ''
            }`}
          >
