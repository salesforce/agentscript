...
import type { GraphNode, PhaseType } from '../../ast/ast-to-graph';
...
export function PhaseNode({ data, selected }: NodeProps<GraphNode>) {
  ...
  if (isEmpty) {
    return (
      <div className="relative min-w-65 rounded-xl border border-dashed border-gray-400 bg-white/50 shadow-none dark:border-zinc-600 dark:bg-zinc-800/50">
        <NodeHandles
          sides={sides}
  ...
  return (
    <div
      className={`relative min-w-65 rounded-xl border border-gray-300 bg-white/50 shadow-none dark:border-zinc-700 dark:bg-zinc-800/50 ${borderClass}`}
    >
      <NodeHandles
        sides={sides}
        connectedHandles={data.connectedHandles}
