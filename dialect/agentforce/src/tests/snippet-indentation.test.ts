...
/**
 * Indentation guardrails for completion snippets in Agentforce `.agent` files.
 *
 ...
 *    existing step."
 *
 * This file is the Agentforce-dialect mirror of
...
describe('snippet indentation — top-level fields (cursor 0)', () => {
   ...
   * cursor 0 with the generator's default step (4), this is correct.
   */
  ...
  it('config: top-level body lines at multiples of 4 from cursor 0', () => {
    const source = '';
    const cand = getCandidate(source, 0, 0, 'config');
    const rendered = stripSnippetMarkers(applyCursorIndent(cand.snippet!, 0));
    const indents = bodyIndents(rendered);
    expect(indents.length).toBeGreaterThan(0);
    for (const indent of indents) {
...
// ---------------------------------------------------------------------------
// Scope 2: Action-level fields inside topic — primary bug repro
// ---------------------------------------------------------------------------
...
describe('snippet indentation — action-level fields (W-22181425 repro)', () => {
   ...
   * inside `topic <name>: → actions: → <action>:`). Completing `inputs:`
   * should produce body lines at cursor + 1*docStep and cursor + 2*docStep.
   * Today they land at cursor + 4 and cursor + 8 (generator hardcoded 4).
   */
  ...
  /**
   * Mirror case for `outputs:` — same bug surface, different field.
   */
...
describe('snippet indentation — top-level fields in a 2-space document', () => {
  /**
   * `variables:` at root in a 2-space doc. The first body line (entry name)
   * should sit at indent 2 (1*docStep), not 4 (1*generator-step). Today
   * fails — the body is at 4 and 8.
   */
  ...
  /**
   * `config:` at root in a 2-space doc. Body lines should sit at indent 2,
   * not 4. Fails today.
   */
  it('config: snippet body uses doc-step (2-space doc, cursor 0)', () => {
    const docStep = 2;
    const source = ['variables:', '  count: mutable number', ''].join('\n');
    const lines = source.split('\n');
    const lastLine = lines.length - 1;
    const cand = getCandidate(source, lastLine, 0, 'config');
    const rendered = stripSnippetMarkers(applyCursorIndent(cand.snippet!, 0));
    const indents = bodyIndents(rendered);
...
describe('snippet indentation — internal step consistency', () => {
  ...
  it('config (top-level) — single step throughout', () => {
    const cand = getCandidate('', 0, 0, 'config');
    assertSingleStep(cand.snippet, 'config');
  });
