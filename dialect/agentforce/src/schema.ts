...
const ContextUserProfileBlock = Block('ContextUserProfileBlock', {
  enabled: BooleanValue.describe(
    "Specifies whether user profile is enabled for the agent. When user profile is enabled, the agent can use the end user's profile attributes as context."
  ),
}).describe(
  "The configuration for user profile capabilities that allow the agent to use the end user's profile attributes as context."
);

const ContextPastConversationsBlock = Block('ContextPastConversationsBlock', {
  enabled: BooleanValue.describe(
    'Specifies whether conversation history is enabled for the agent. When conversation history is enabled, the agent can use past conversations with the end user as context.'
  ),
}).describe(
