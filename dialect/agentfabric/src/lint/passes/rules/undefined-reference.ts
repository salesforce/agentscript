...
function resolveCheck(
  ...
  // Nodes claimed by constraint checks (e.g. `allowedNamespaces` on a router
  // route `target:`) are added to `validatedRefs`. Constraint validation only
  // checks that the *namespace* is allowed (and, for `resolvedType`, that the
  // namespace has the right capability) — it does NOT verify the referenced
  // *member* exists. So `@generator.missingRoute` in a route target passes
  // constraint validation (namespace `generator` is allowed) yet names a
  // generator that was never declared.
  //
  // Previously any `validatedRefs` node was skipped outright, letting those
  // member-level typos through. Instead, skip only when constraint validation
  // already attached its own diagnostic to the node (a bad namespace /
  // capability — flagging again would duplicate it). When no constraint
  // diagnostic is present the namespace is valid, so fall through and let the
  // normal resolution below verify the member exists.
  if (validatedRefs.has(expr)) {
    const hasConstraintDiagnostic = (expr.__diagnostics ?? []).some(
      d => typeof d.code === 'string' && d.code.startsWith('constraint-')
    );
    if (hasConstraintDiagnostic) return { kind: 'skip-validated' };
  }
  ...
  // The namespace IS a real schema key, but nothing in the document declares
  // any members for it (candidates === null). Any `@<namespace>.<property>`
  // reference is therefore undefined — the block/node it names does not exist.
  //
  // This closes a uniform gap: this branch previously bailed silently, so
  // references into a wholly-undefined namespace (e.g. `@variables.foo`
  // with no `variables:` block, or `@subagent.bar` with no subagents) were
  // silently accepted. We now flag them with the standard per-member miss.
  // Candidates are empty (nothing is defined), so no "did you mean" suggestion
  // is offered.
  //
  // Note: `@actions` never reaches this branch. It is a *scoped* namespace
  // (see `scopedNamespaces`) and AgentFabric marks no scope as colinearly
  // resolvable, so a bare unresolved `@actions.X` is intercepted ~20 lines
  // above by the `isScopedNs` branch and returns `non-referenceable-scope`
  // (message: "'@actions' cannot be used as a reference..."). That message is
  // stripped by suppressActionsNamespaceUndefinedReferencePass via its
  // "'@actions' cannot be used as a reference" filter — NOT the separate
  // "is not defined in actions" filter (which covers mistyped members of a
  // *declared* action set, i.e. the `candidates !== null` standard-miss path).
  // Do not assume this return handles `@actions`.
  return { kind: 'standard-miss', candidates: [] };
}
