**AgentScript API** • **Docs**

---

# AgentScript API

## Classes

- [WorkerParserManager](classes/WorkerParserManager.md)

## Interfaces

- [ErrorResult](interfaces/ErrorResult.md)
- [ExtensionManifest](interfaces/ExtensionManifest.md)
- [HighlightCapture](interfaces/HighlightCapture.md)
- [HighlightResult](interfaces/HighlightResult.md)
- [LspExtensionConfig](interfaces/LspExtensionConfig.md)
- [ParseError](interfaces/ParseError.md)
- [ParseResult](interfaces/ParseResult.md)
- [SchemaFieldInfo](interfaces/SchemaFieldInfo.md)
- [SerializedNode](interfaces/SerializedNode.md)
- [ThemeColors](interfaces/ThemeColors.md)
- [TokenStyle](interfaces/TokenStyle.md)
- [VscodeWrapper](interfaces/VscodeWrapper.md)

## Type Aliases

- [HoverInfo](type-aliases/HoverInfo.md)
- [KeywordHoverInfo](type-aliases/KeywordHoverInfo.md)
- [SchemaHoverInfo](type-aliases/SchemaHoverInfo.md)

## Functions

- [buildMonacoRules](functions/buildMonacoRules.md)
- [buildVscodeRules](functions/buildVscodeRules.md)
- [clearCrashCache](functions/clearCrashCache.md)
- [createDiagnosticMarkers](functions/createDiagnosticMarkers.md)
- [createHoverProvider](functions/createHoverProvider.md)
- [createLspExtension](functions/createLspExtension.md)
- [disableParser](functions/disableParser.md)
- [enableParser](functions/enableParser.md)
- [getHighlightCaptures](functions/getHighlightCaptures.md)
- [initializeParser](functions/initializeParser.md)
- [initializeTreeSitter](functions/initializeTreeSitter.md)
- [isParserDisabled](functions/isParserDisabled.md)
- [isParserReady](functions/isParserReady.md)
- [parseAgentScript](functions/parseAgentScript.md)
- [parseAndGetErrors](functions/parseAndGetErrors.md)
- [registerAgentScriptLanguage](functions/registerAgentScriptLanguage.md)
- [resetParser](functions/resetParser.md)
- [resolveHoverInfo](functions/resolveHoverInfo.md)
- [terminateParser](functions/terminateParser.md)

## Variables

- [darkTheme](variables/darkTheme.md)
- [darkThemeColors](variables/darkThemeColors.md)
- [languageConfiguration](variables/languageConfiguration.md)
- [lightTheme](variables/lightTheme.md)
- [lightThemeColors](variables/lightThemeColors.md)
- [theme](variables/theme.md)
- [workerParser](variables/workerParser.md)
